...
This project also includes a *simple* disk image converter that takes one of  `*.HFE`, `*.RAW`, `*.MFM`, `*.D77` or `*.FDX` disk image data and convert it into either one of `*.MFM`, `*.D77` or `*.FDX` disk image data.  
...
|`-o`|`filename`|Specify output file. MFM, D77 and FDX are supported.|
